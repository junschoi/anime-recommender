...
Currently, the data is stored locally, and the flask app is also hosted locally. However, local hosting poses a lot of potential problems. Migrating the whole structure to a single or multiple cloud platforms would greatly increase the app's stability and accessibility. Finally, Jikan API showed its flaws in speed and stability during the data collection process. Coming up with a better data collection method would save time, and creating cron jobs to periodically update anime data automatically would eliminate the need to manually run data collection scripts. 
